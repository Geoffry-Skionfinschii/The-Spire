...
execute as @a[scores={SPWN_ClickSign=1..}] run function infection_spawn_building:sign_click/find_sign

scoreboard players add $GlassTimer SPWN_SpawnStuff 1

execute if score $GlassTimer SPWN_SpawnStuff matches 200.. at @e[type=marker,limit=1,tag=GEN_SPAWN_MARKER] run function infection_spawn_building:change_glass
